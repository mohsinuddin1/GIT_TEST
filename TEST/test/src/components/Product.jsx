...
const Product = ({product}) => {
  ...
  return (
    <div >
      ...
      relative overflow-hidden group transition shadow-sm'>
        ...
        items-center'>
         ...
         group-hover:opacity-100 transition-all duration-300'>
          <button onClick={()=>addToCart(product,id)}>
            ...
            <Link to={`/product/${id}`}
              className='w-12 h-12 bg-slate-300 flex justify-center 
              drop-shadow-xl items-center
               font-bold'>see</Link>
         </div>
        </div>
      </div>
